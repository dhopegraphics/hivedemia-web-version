...
export const usePlannerStore = create((set, get) => ({
  ...
  initPlannerTables: async () => {
    try {
      await dbManager.executeWithRetry("planner.db", async (db) => {
        await db.execAsync(`
  ...
  loadPlannerFromDB: async () => {
    try {
      await dbManager.executeWithRetry("planner.db", async (db) => {
        ...
        const tasksRaw = await db.getAllAsync(
        ...
        const tasks = await Promise.all(
          tasksRaw.map(async (plan) => {
            const taskItems = await db.getAllAsync(
        ...
        const plansRaw = await db.getAllAsync(
  ...
  deleteTask: async (taskId, planId) => {
    try {
      await dbManager.executeWithRetry("planner.db", async (db) => {
        ...
        await db.runAsync(`DELETE FROM task_items WHERE task_id = ?`, taskId);

        // Step 2: Check if any tasks remain in the plan
        const remainingTasks = await db.getAllAsync(
          `SELECT * FROM task_items WHERE plan_id = ?`,
          planId
        );
        ...
        if (remainingTasks.length === 0) {
          await db.runAsync(`DELETE FROM tasks WHERE plan_id = ?`, planId);
        }
      });
  ...
  updateExam: async (examData) => {
    try {
      await dbManager.executeWithRetry("planner.db", async (db) => {
        await db.runAsync(
          `UPDATE exams SET date = ?, time = ?, location = ?, priority = ? WHERE id = ?`,
          [
            examData.date,
            examData.time,
            examData.location,
  ...
  deleteWeeklyPlan: async (planId) => {
    try {
      await dbManager.executeWithRetry("planner.db", async (db) => {
        ...
        await db.runAsync(
          `DELETE FROM weekly_entries WHERE plan_id = ?`,
          planId
        );
        // Then delete the plan
        await db.runAsync(`DELETE FROM weekly_plans WHERE id = ?`, planId);
      });
