...
import { examsDb } from '@/data/localDb';
...
interface ExamState {
  ...
  createExam: (exam: Omit<Exam, 'id' | 'createdAt'>, topics: { id: number; name: string }[]) => Promise<string>;
  saveExamQuestions: (examId: string, questions: Omit<ExamQuestion, 'id' | 'examId'>[]) => Promise<void>;
...
export const useExamStore = create<ExamState>((set) => ({
    ...
    try {
      ...
      await examsDb.runAsync(
        `INSERT INTO exams (id, userId, title, subject, duration, questionCount, difficulty, includeTimer, shuffleQuestions, timePerQuestion, createdAt) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, exam.userId, exam.title, exam.subject, exam.duration, 
         exam.questionCount, exam.difficulty, exam.includeTimer ? 1 : 0, 
         exam.shuffleQuestions ? 1 : 0, exam.timePerQuestion, createdAt]
      );
 ...
 saveExamQuestions: async (examId, questions) => {
  ...
  getExamsByUser: async (userId) => {
    try {
      set({ isLoading: true });
      const exams = await examsDb.getAllAsync(
  ...
  getExamWithDetails: async (examId) => {
    try {
      ...
      const exam = await examsDb.getFirstAsync(
        'SELECT * FROM exams WHERE id = ?',
        [examId]
      );
      
      const topics = await examsDb.getAllAsync(
        'SELECT * FROM exam_topics WHERE examId = ?',
        [examId]
      );
      
      const questions = await examsDb.getAllAsync(
  ...
  deleteExam: async (examId) => {
    try {
      set({ isLoading: true });
      await examsDb.runAsync('DELETE FROM exams WHERE id = ?', [examId]);
      set((state) => ({
...
`;
      ...
      try {
        ...
        examQuestions = examQuestions.map((q: any) => ({
          ...q,
          options: q.type === 'shortAnswer' ? '[]' : JSON.stringify(q.options || [])
        }));
