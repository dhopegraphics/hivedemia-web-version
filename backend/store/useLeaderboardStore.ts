...
async function getLocalLeaderboard() {
  ...
  const rows = await db.getAllAsync<LeaderboardUser>('SELECT * FROM leaderboard');
  return rows.map(row => ({
    ...row,
...
async function setLocalLeaderboard(data: LeaderboardUser[]) {
  ...
  for (const item of data) {
    await db.runAsync(
      ...
      item.id, item.name, item.score, item.avatar, item.streak, item.isCurrentUser ? 1 : 0
    );
  }
}
...
async function setLocalBadges(data: Badge[]) {
  ...
  for (const item of data) {
    await db.runAsync(
      ...
      item.id, item.name, item.icon, item.earned ? 1 : 0
    );
  }
}
...
async function getLocalStreak() {
  const db = await getDb();
  const rows = await db.getAllAsync<{ value: number }>('SELECT value FROM streak WHERE id = 1');
  ...
  syncAll: async () => {
    try {
      ...
      const { data: participants, error: partErr } = await supabase
        .from("competition_participants")
        .select("user_id, score, completed, joined_at, completed_at, profiles(full_name, username, avatar_url)")
