...
export interface DocumentItem {
  id: number | string;
  url: string;
  filePath: string; // Optional path for local files
...
interface DocumentProcessingState {
  ...
  loadExtractedTopicsFromLocalDB: (
    fileIds: string[]
  ) => Promise<Record<string, TopicExtractionResult>>;
}
...
export const useAIDocumentProcessor = create<DocumentProcessingState>(
  (set, get) => ({
      ...
      while (retries > 0) {
        try {
          const requestBody: ProcessDocumentRequest = {
            fileId: item.id,
            fileUrl: item.unprocessedUrl, // Use Google Drive URL for Edge function
            fileName: item.name,
            fileType: item.type,
          ...
          if (response.success) {
            ...
            if (action === "analyze_document") {
            ...
            } else if (action === "extract_topics" && response.data?.topics) {
              await localStore.saveExtractedTopics(
                item.id.toString(),
                item.courseId,
                response.data.topics,
              ...
              // Also save to Supabase to ensure consistency
              try {
                await localStore.saveTopicsToSupabase(
                  item.id.toString(),
                  item.courseId,
                  response.data.topics
                );
                ...
                console.warn(
                  "Failed to save topics to Supabase, but local save succeeded:",
                  supabaseError
                );
              }
            }
    ...
    analyzeDocument: async (item, customMessage) => {
      console.log("Analyzing document:", item);
    ...
    loadExtractedTopicsFromLocalDB: async (fileIds) => {
      const { useAIDocumentLocalStore } = await import(
        "./useAIDocumentLocalStore"
      );
      const localStore = useAIDocumentLocalStore.getState();

      const extractedTopicsFromDB: Record<string, TopicExtractionResult> = {};

      // Load from local DB for each file
      for (const fileId of fileIds) {
        try {
          const topicsData = await localStore.getExtractedTopicsByFile(fileId);
          if (topicsData && topicsData.topics) {
            extractedTopicsFromDB[fileId] = {
              topics: topicsData.topics,
              summary: topicsData.summary,
              topicsCount: topicsData.topicsCount || topicsData.topics.length,
            };
          }
        } catch (error) {
          console.error(`Error loading topics for file ${fileId}:`, error);
        }
      }

      // Update the in-memory state with loaded data
      set((state) => ({
        extractedTopicsByFile: {
          ...state.extractedTopicsByFile,
          ...extractedTopicsFromDB,
        },
      }));

      return extractedTopicsFromDB;
    },
  })
);
