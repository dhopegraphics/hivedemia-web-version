...
export const useQuizStore = create<QuizState>((set) => ({
  ...
  initializeDatabase: async () => {
    try {
      await dbManager.executeWithRetry("quizzes.db", async (db) => {
        set({ isLoading: true });
        await db.execAsync(`
  ...
  getQuizzesByUser: async (userId) => {
    try {
      set({ isLoading: true });
      const quizzes = (await dbManager.executeWithRetry(
        "quizzes.db",
        async (db) => {
  ...
  getQuizWithQuestions: async (quizId) => {
    try {
      set({ isLoading: true });
      const quiz = (await dbManager.executeWithRetry(
        "quizzes.db",
        async (db) => {
          return await db.getFirstAsync("SELECT * FROM quizzes WHERE id = ?", [
            quizId,
          ]);
        }
      )) as Quiz | null;

      const rawQuestions = (await dbManager.executeWithRetry(
        "quizzes.db",
        async (db) => {
          return await db.getAllAsync(
      ...
      const questions: QuizQuestion[] = rawQuestions.map((question: any) => {
        return {
          ...question,
          options: (() => {
            if (!question.options) {
  ...
  deleteQuiz: async (quizId) => {
    try {
      ...
      const quizzesDb = await SQLite.openDatabaseAsync("quizzes.db");

      // Delete questions first, then quiz (foreign key constraint)
      const questionsResult = await quizzesDb.runAsync(
      ...
      const quizResult = await quizzesDb.runAsync(
  ...
  updateQuizTitle: async (quizId, newTitle) => {
    try {
      const quizzesDb = await SQLite.openDatabaseAsync("quizzes.db");
      await quizzesDb.runAsync("UPDATE quizzes SET prompt = ? WHERE id = ?", [
        newTitle,
        quizId,
      ]);
      set((state) => ({
