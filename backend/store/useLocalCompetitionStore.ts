  ...
  (set, get) => ({
    ...
    initializeDB: async () => {
      try {
        ...
        const db = await SQLite.openDatabaseAsync("competition.db");
    ...
    preloadCompetitionData: async (competitionId) => {
      const { db } = get();
      if (!db) throw new Error("Database not initialized");
      ...
      try {
        ...
        // Fetch from online store - now using fetchAllQuestionAnswers
        ...
        try {
          // Insert questions
          for (const question of onlineStore.competitionQuestions) {
            await db.runAsync(
          ...
          // Insert answers - now using the pre-fetched questionAnswersByQuestionId
          for (const [questionId, answers] of Object.entries(
            onlineStore.questionAnswersByQuestionId
          )) {
