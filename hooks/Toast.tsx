...
const Toast = forwardRef<IToast, Props>(({ duration = 400, onHide }, ref) => {
  ...
  }, [config.duration, transY, transX, visibleState]);
  ...
  useEffect(() => {
    if (textLength > 0 && toastHeight > 0 && config.text) {
      transX.value = withTiming(textLength + 24, { duration: 200 });

      // Ensure animation starts only after `textLength` is ready
      setTimeout(() => {
        showToast();
      }, 50);
  ...
  }, [config, toastHeight, textLength, hideToast, showToast, transX]);
